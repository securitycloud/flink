 ...
 * @author Lastovicka
 */
public class AggregateTest extends RichMapFunction<Flow, String>{

    private static int counter;
    private static final Map<String, Integer> aggregate = new HashMap<>();
    ParameterTool parameterTool;

    private static final AggregateTest singleton = new AggregateTest();

    private AggregateTest() {
        Logger.getLogger(AggregateTest.class.getName()).log(Level.INFO, "aggregate constructor called");
        AggregateTest.counter = 0;
        try {
            ...
            Logger.getLogger(AggregateTest.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

    public static AggregateTest getInstance() {
        return singleton;
    }
    ...
    public String map(Flow flow) throws Exception {
        ...
        if ((counter % parameterTool.getInt("countwindow.size")) == 0) {
            ...
            aggregate.clear();
            return "aggregate " + String.valueOf(System.currentTimeMillis()) + " " + String.valueOf(parameterTool.getInt("countwindow.size")) + " " + map;
